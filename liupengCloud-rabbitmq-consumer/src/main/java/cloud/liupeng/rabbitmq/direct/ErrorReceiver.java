 ...
 * @QueueBinding注解->value：绑定队列的名称。exchange：配置交换器。key：配置路由键
 * @Queue注解->value：配置队列名称。autoDelete：是否为一个可删除的临时队列
 * @Exchange注解->value：配置交换器名称。type：指定具体的交换器类型
 * @author liupeng
 *
 */
@Component
...
public class ErrorReceiver {
	...
	public void process(String message) {
		...
		System.out.println("接收处理...error...消息：" + message);
	}
}
